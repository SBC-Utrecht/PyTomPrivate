...
#
# # @jit(nopython=b)
# @vectorize(["float32(float32, float32, float32, float32, float32, float32, float32, float32)"], nopython=True)
...
#
# # @jit(nopython=b)
# @guvectorize(["float32[:,:,:], float32, float32, float32, float32[:]"], "(k,l,m), (), (), () -> ()", nopython=b, target="cpu")
# def linearInterpolation(data, x, y, z, out):
...
#         #src += this->stridey
#         x01x = data[floorx][floory+1][floorz] + (data[floorx+1][floory+1][floorz]-data[floorx][floory+1][floorz])*xoffseth
#         #src += this->stridez;
#         x11x = data[floorx][floory+1][floorz+1] + (data[floorx+1][floory+1][floorz+1]-data[floorx][floory+1][floorz+1])*xoffseth
#         #src -= this->stridey;
#         #floory -= 1
#         x10x = data[floorx][floory][floorz+1] + (data[floorx+1][floory][floorz+1]-data[floorx][floory][floorz+1])*xoffseth #src[0] + (src[1]-src[0])*xoffseth;
...
#         #src += this->stridez;
#         out[0] = x0y0 + (data[floorx][floory][floorz+1] + (data[floorx][floory+1][floorz+1]-data[floorx][floory][floorz+1])*yoffseth - x0y0)*zoffseth
#
#     elif case == 5:
#         x00x = data[floorx][floory][floorz] + (data[floorx+1][floory][floorz]-data[floorx][floory][floorz])*xoffseth
#         #src += this->stridez;
#         out[0] = x00x + (data[floorx][floory][floorz+1] + (data[floorx+1][floory][floorz+1]-data[floorx][floory][floorz+1])*xoffseth - x00x)*zoffseth
#     elif case == 3:
#         x00x = data[floorx][floory][floorz] + (data[floorx+1][floory][floorz]-data[floorx][floory][floorz])*xoffseth
#         #src += this->stridey;
#         out[0] = x00x + (data[floorx][floory+1][floorz] + (data[floorx+1][floory+1][floorz]-data[floorx][floory+1][floorz])*xoffseth - x00x)* yoffseth
#
#     elif case == 4:
#         out[0] = data[floorx][floory][floorz] + (data[floorx][floory][floorz+1] - data[floorx][floory][floorz])*zoffseth
#     elif case == 2:
#         out[0] = data[floorx][floory][floorz] + (data[floorx][floory+1][floorz] - data[floorx][floory][floorz])*yoffseth
#     elif case == 1:
#         out[0] = data[floorx][floory][floorz] + (data[floorx+1][floory][floorz] - data[floorx][floory][floorz])*xoffseth
#     else:
#         out[0] = data[floorx][floory][floorz]
#
# # @jit(nopython=b)
# @guvectorize(["float32[:,:,:], float32, float32, float32, float32[:]"], "(k,l,m), (), (), () -> ()", nopython=b, target="cpu")
# def cubicInterpolation(data, x, y, z, out):
#     is3D = data.shape[2] > 1
#     sizex,sizey,sizez = data.shape
#
#
...
#         out[0] = 0.#linearInterpolation(data, x,y,z)
#
...
#     layerOffset = 1
#
#     layerValues = xp.array((0,0,0), dtype=xp.float32)
#
#     if( not is3D):
#         lowerLayerBound = 0
#         upperLayerBound = 0
...
#
#     out[0] = (line1 + line2 + line3)

# @jit(nopython=b)
# @guvectorize(["void(float32[:,:,:], float32, float32, float32, float32[:])"], "(k,l,m), (), (), () -> ()", nopython=b, target="cpu")
# def splineInterpolation(data, x, y, z, out):
...
#         out[0] = 0.#cubicInterpolation(data, x, y, z)
#
#
...
#     upperLayerBound = 2
#     layerOffset = 1
#     layerValues = xp.zeros((4), dtype=data.dtype)
#
#     if( not is3D):
#         lowerLayerBound = 0
#         upperLayerBound = 0
...
# @jit(nopython=b)
# @guvectorize(["complex64[:,:,:], float32, float32, float32, float32[:]"], "(k,l,m), (), (), () -> ()", nopython=b, target="cpu")
# def fourierSplineInterpolation(data, x, y, z, out):
#     sizex, sizey = data.shape[:2]
#
#     is3D = len(data.shape) == 3
#
...
#
#         # is the current position in the data or outside. return default value if outside
#         if (x<2. or y<2. or z<2. or x > sizex-3 or y > sizey-3 or z > sizez-3):
#             out[0] = 0 + 0j
#
#
#     if (not is3D and (x<2. or y<2. or x > (sizex-3) or y > (sizey-3))):
...
@njit(parallel=nopython)
def fill_values_real_linear(src, dst, mtx, dims_src,  dims):
    for dx in range(dims[0]):
        for dy in range(dims[1]):
            for dz in range(dims[2]):
                ...
                v = None
                tooClose4Linear3D = (is3D and (x < 0. or y < 0. or z < 0. or x >= (sizex-1) or y >= (sizey-1) or z >= (sizez-1)))
                tooClose4Linear2D = (not is3D and (x < 0. or y < 0. or x >= (sizex-1) or y >= (sizey-1)))

                if tooClose4Linear2D or tooClose4Linear3D:
                    v = 0
                ...
                if v is None:
                    ...
                    case = (xoffseth > 0.000001)*1 + (yoffseth > 0.000001)*2 + (zoffseth>0.000001)*4
                    if case == 7:
                        x00x = src[floorx][floory][floorz] + (src[floorx+1][floory][floorz] - src[floorx][floory][floorz])*xoffseth
                        #src += this->stridey
                        x01x = src[floorx][floory+1][floorz] + (src[floorx+1][floory+1][floorz]-src[floorx][floory+1][floorz])*xoffseth
                        #src += this->stridez;
                        x11x = src[floorx][floory+1][floorz+1] + (src[floorx+1][floory+1][floorz+1]-src[floorx][floory+1][floorz+1])*xoffseth
                        #src -= this->stridey;
                        #floory -= 1
                        x10x = src[floorx][floory][floorz+1] + (src[floorx+1][floory][floorz+1]-src[floorx][floory][floorz+1])*xoffseth #src[0] + (src[1]-src[0])*xoffseth;
                        x0yx = x00x + (x01x - x00x) * yoffseth
                        ...
                        x0y0 = src[floorx][floory][floorz] + (src[floorx][floory+1][floorz]-src[floorx][floory][floorz])*yoffseth
                        #src += this->stridez;
                        v = x0y0 + (src[floorx][floory][floorz+1] + (src[floorx][floory+1][floorz+1]-src[floorx][floory][floorz+1])*yoffseth - x0y0)*zoffseth

                    elif case == 5:
                        x00x = src[floorx][floory][floorz] + (src[floorx+1][floory][floorz]-src[floorx][floory][floorz])*xoffseth
                        #src += this->stridez;
                        v = x00x + (src[floorx][floory][floorz+1] + (src[floorx+1][floory][floorz+1]-src[floorx][floory][floorz+1])*xoffseth - x00x)*zoffseth
                    elif case == 3:
                        x00x = src[floorx][floory][floorz] + (src[floorx+1][floory][floorz]-src[floorx][floory][floorz])*xoffseth
                        #src += this->stridey;
                        v = x00x + (src[floorx][floory+1][floorz] + (src[floorx+1][floory+1][floorz]-src[floorx][floory+1][floorz])*xoffseth - x00x)* yoffseth

                    elif case == 4:
                        v = src[floorx][floory][floorz] + (src[floorx][floory][floorz+1] - src[floorx][floory][floorz])*zoffseth
                    elif case == 2:
                        v = src[floorx][floory][floorz] + (src[floorx][floory+1][floorz] - src[floorx][floory][floorz])*yoffseth
                    elif case == 1:
                        v = src[floorx][floory][floorz] + (src[floorx+1][floory][floorz] - src[floorx][floory][floorz])*xoffseth
                    else:
...
@njit(parallel=nopython)
def fill_values_real_cubic(src, dst, mtx, dims_src, dims):
    for dx in range(dims[0]):
        for dy in range(dims[1]):
            for dz in range(dims[2]):
                x = mtx[0] * dx + mtx[1] * dy + mtx[2] * dz + mtx[3]
                ...
                if v is None:
                    ...
                    if v is None:
                        ...
                        layerValues = [0.,0.,0.] # xp.array((0,0,0), dtype=xp.float32)
...
@njit(parallel=nopython)
def fill_values_real_fourierSpline(src, dst, mtx, dims_src, dims):
    for dx in range(dims[0]):
        for dy in range(dims[1]):
            for dz in range(dims[2]):
                # x = i // (dims[1] * dims[2])
                ...
                # is the current position in the data or outside. return default value if outside
                a = (is3D and (x < 2. or y < 2. or z < 2. or x > sizex - 3 or y > sizey - 3 or z > sizez - 3))
                b = (not is3D and (x < 2. or y < 2. or x > (sizex - 3) or y > (sizey - 3)))
                ...
                if a or b:
                    ...
                    if v is None:
                        ...
                        if v is None:
                            ...
                            layerValues = [0.+0j, 0.+0j, 0.+0j] # xp.array((0,0,0), dtype=src.dtype)
...
@njit(parallel=nopython)
def fill_values_complex(src, dst, mtx, dims_src, dims, shiftX, shiftY, shiftZ):
    center_x = dims[0] // 2
    center_y = dims[1] // 2
    center_z = dims[2] // 2

    dimsR = [dims[0], dims[1], dims[2] // 2 + 1]

    for dx in range(dimsR[0]):
        for dy in range(dimsR[1]):
            for dz in range(dimsR[2]):
                x = mtx[0] * dx + mtx[1] * dy + mtx[2] * dz + mtx[3]
                y = mtx[4] * dx + mtx[5] * dy + mtx[6] * dz + mtx[7]
                z = mtx[8] * dx + mtx[9] * dy + mtx[10] * dz + mtx[11]
                ...
                s = complex(xp.cos(2 * M_PI / sizex * dx * shiftX), -xp.sin(2 * M_PI / sizex * dx * shiftX)) * \
                    complex(xp.cos(2 * M_PI / sizey * dy * shiftY), -xp.sin(2 * M_PI / sizey * dy * shiftY)) * \
                    complex(xp.cos(2 * M_PI / sizez * dz * shiftZ), -xp.sin(2 * M_PI / sizez * dz * shiftZ))

                dst[dx,dy,dz] = v * s
                rx, ry, rz = 2 * center_x - x, 2 * center_y - y, 2 * center_z - z

                if rx < dims[0] and ry < dims[1] and rz < dims[2]:
                    dst[rx][ry][rz] = dst[dx,dy,dz].conj()



# @jit(nopython=b)
# @guvectorize(["float32[:,:,:], float32[:,:,:], float32[:,:], float32[:]"], "(k,l,m), (n,o,p), (a,b), (c) ", nopython=b, target="cpu")
# def fill_values_real_cubic(src, dst, mtx, dims):
#     for i in range(dst.size):
#         x = i // (dims[1] * dims[2])
#         y = (i // dims[2]) % dims[1]
#         z = (i % dims[2])
#
#         dx = mtx[0] * x + mtx[1] * y + mtx[2] * z + mtx[3]
#         dy = mtx[4] * x + mtx[5] * y + mtx[6] * z + mtx[7]
#         dz = mtx[8] * x + mtx[9] * y + mtx[10] * z + mtx[11]
#         dst[x,y,z] = cubicInterpolation(src, dx, dy, dz)
#
# # @jit(nopython=b
# @guvectorize(["float32[:,:,:], float32[:,:,:], float32[:,:], float32[:]"], "(k,l,m), (n,o,p), (a,b), (c) ", nopython=b, target="cpu")
# def fill_values_real_linear(src, dst, mtx, dims):
#     for i in range(dst.size):
#         x = i // (dims[1] * dims[2])
#         y = (i // dims[2]) % dims[1]
#         z = (i % dims[2])
...
#         dy = mtx[4] * x + mtx[5] * y + mtx[6] * z + mtx[7]
#         dz = mtx[8] * x + mtx[9] * y + mtx[10] * z + mtx[11]
#         dst[x,y,z] = linearInterpolation(src, dx, dy, dz)
#
# # @jit(nopython=b)
# @guvectorize(["float32[:,:,:], float32[:,:,:], float32[:,:], float32[:]"], "(k,l,m), (n,o,p), (a,b), (c) ", nopython=b, target="cpu")
# def fill_values_real_fourierSpline(src, dst, mtx, dims):
#     for i in range(dst.size):
#         x = i // (dims[1] * dims[2])
#         y = (i // dims[2]) % dims[1]
#         z = (i % dims[2])
...
#         dy = mtx[4] * x + mtx[5] * y + mtx[6] * z + mtx[7]
#         dz = mtx[8] * x + mtx[9] * y + mtx[10] * z + mtx[11]
#         dst[x,y,z] = fourierSplineInterpolation(src, dx, dy, dz)
#
# # @jit(nopython=b)
# @guvectorize(["complex64[:,:,:], complex64[:,:,:], float32[:,:], float32[:], complex64[:,:]"], "(k,l,m), (n,o,p), (a,b), (n,o,p)", nopython=b, target="cpu")
# def fill_values_complex(src, dst, mtx, dims, shift):
#     center_x = dims[0] // 2
#     center_y = dims[1] // 2
#     center_z = dims[2] // 2
#
#     dimsR = [dims[0], dims[1], dims[2] // 2 + 1]
#     size = dst.shape[0] * dst.shape[1] * (dst.shape[2] // 2 + 1)
#
#     for i in range(size):
#         x = i // (dimsR[1] * dimsR[2])
#         y = (i // dimsR[2]) % dimsR[1]
...
#         dy = mtx[4] * x + mtx[5] * y + mtx[6] * z + mtx[7]
#         dz = mtx[8] * x + mtx[9] * y + mtx[10] * z + mtx[11]
#         #if z == 25 and (dx > dims[0]-3.5 or dy > dims[1]-3): continue
#
#         s = 1 if shift is None else shift[x][y][z]
#
#         dst[x,y,z] = fourierSplineInterpolation(src, dx, dy, dz) * s
#         rx, ry, rz = 2 * center_x - x, 2 * center_y - y, 2 * center_z - z
#         #if rz ==25 and (rx > dims[0]-4 or ry > dims[1]-4): continue
#         if rx < dims[0] and ry < dims[1] and rz < dims[2]:
#             dst[rx][ry][rz] = dst[x,y,z].conj()