...
if __name__ == '__main__':
    ...
    helper = ScriptHelper(sys.argv[0].split('/')[-1],
                          ...
                          options=[ScriptOption(['-t', '--tomogram'],
                                                'Reconstruct a tomogram. Specify name of tomogam here. You do not need a particle list for that!',
                                                arg=True, optional=True),
                                   ScriptOption(['-p', '--particleList'], 'XML particle list.', arg=True,
                                                optional=False),
                                   ScriptOption(['--projectionList'], 'XML projection list.', arg=True, optional=True),
                                   ScriptOption(['--projectionDirectory'], 'Directory containing the projections.',
                                                arg=True, optional=False),
                                   ScriptOption(['-w', '--applyWeighting'],
                                                'If projections are not weighted, apply weighting before. If omited, no weighting.',
                                                arg=False, optional=True),
                                   ScriptOption(['-s', '--size'], 'Size of particle cube / tomogram.', arg=True,
                                                optional=False),
                                   ScriptOption(['-b', '--coordinateBinning'],
                                                'Binning factor of coordinates. If particle coordinates are determined in binned volume (with respect to projections) this binning factor needs to be specified.',
                                                arg=True, optional=True),
                                   ScriptOption(['-o', '--recOffset'], 'Cropping offset of the binned tomogram.',
                                                arg=True, optional=False),
                                   ScriptOption(['--projBinning'],
                                                'Bin projections BEFORE reconstruction. 1 is no binning, 2 will merge two voxels to one, 3 -> 1, 4 ->1 ...',
                                                arg=True, optional=True),
                                   ScriptOption(['-a', '--alignResultFile'],
                                                'Use an alignmentResultFile to generate the aligned files in memory.',
                                                arg=True, optional=True),
                                   ScriptOption(['-r', '--particlePolishFile'],
                                                'Use an particlePolishFile to generate the polished cutouts.', arg=True,
                                                optional=True),
    ...
    try:
        tomogram, particleListXMLPath, projectionList, projectionDirectory, aw, size, coordinateBinning, recOffset, projBinning, alignmentResultFile, particlePolishFile, help = parse_script_options(
            sys.argv[1:], helper)
    ...
    if tomogram:
        ...
        for n, particle in enumerate(particleList):
            ...
            if particlePolishFile and len(polishedCoordinates['AlignmentTransX']) == len(particleList) * len(
                    projections):
        ...
        projections.reconstructVolumesMultiThreaded(particles=particleList, cubeSize=int(size[0]), \
                                       binning=projBinning, applyWeighting=aw, \
                                       showProgressBar=True, verbose=False, \
                                       preScale=projBinning, postScale=1, alignResultFile=alignmentResultFile, num_procs=20)
